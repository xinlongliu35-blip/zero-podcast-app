...
export default class PetalParticles {
  ...
  createPetalMaterial(perlinTexture) {
    ...
    const uBlackColor = uniform(new THREE.Color('#000000'));
    const uGrayColor = uniform(new THREE.Color('#333333'));
    const uRedColor = uniform(new THREE.Color('#881111')); // Dark red tint
    ...
    // Petals color computation - mix in some red for visual depth
    const basePetalColor = mix(uBlackColor, uGrayColor, instanceIndex.mod(3).equal(0));
    const petalColor = mix(basePetalColor, uRedColor, instanceIndex.mod(7).equal(0));
